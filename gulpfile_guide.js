...
function spriteIcon(done){
    ...
    //폴더없이 메인루트일때
    var spriteData = gulp.src(path.sprite.input + "*.*")
        .pipe(spritesmith({
            imgName: imgName + "png",
    ...
    folders.forEach(function (folder) {
        spriteData = gulp.src(path.sprite.input + folder + '/*.png')
            .pipe(spritesmith({
...
function renderSprite(cb){
	...
	fs.readdir(directoryPath, function (err, files) {
		...
		for (let file of files) {
			...
			//가이드 html 구조
			images +=
			`<div class="spriteContainer__item"><figure class="spriteContainer__img"><img src="${filePath}" alt="${fileNameWithoutExtension}"><figcaption>${fileNameWithoutExtension}</figcaption></figure></div>\n`
		}
