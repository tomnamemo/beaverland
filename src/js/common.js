$(document).ready(function () {
	...
	function closeMenu() {
		$(".header_menu_button").html("MENU"); // 메뉴 버튼 텍스트를 'MENU'로 변경
		var j = 0;

		$($(".header_item").get().reverse()).each(function () {
		...
		$(".header_nav") // 모든 .header_item이 처리된 후에 .header_nav를 숨김
			.delay(300 * j)
			.fadeOut();
	...
	$(".header_menu_button").click(function () {
		...
		if (!isOpen) {
			$(".header_nav").fadeIn();
			$(".header_menu_button").html("CLOSE").css({ backgroundColor: "#fbbf24" });
	...
	// .header_item 안의 a 태그를 클릭하면 해당 섹션으로 이동 후 메뉴가 닫히게 처리
	$(".header_item a").click(function (e) {
	...
	$(window).on("scroll touchmove", function () {
		// 스크롤 위치에 따라 .header_menu_button의 배경색을 왼쪽에서 오른쪽으로 채우기
		...
		$(".header_menu_button").css("background", "linear-gradient(to right, #fbbf24 " + (currentScroll / totalScrollHeight) * 100 + "%, transparent 0%)");
		...
		$(".section_title").each(function () {
			...
			if ($(window).scrollTop() > sectionTop - $(window).height() / 2) {
				$(this).addClass("animate_border");
			} else {
				$(this).removeClass("animate_border");
			}
		});
	...
	// .section_title 모바일용 터치스타트 추가 스크립트
	$(".section_title").on("touchstart", function () {
		$(this).addClass("animate_border");
	});
