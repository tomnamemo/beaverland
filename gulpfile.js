...
const path = {
	styles: {
		input: src_assets + "/sass/",
		inputFile: src_assets + "/sass/**/*.scss",
		output: dist_assets + "/css/",
		autoprefixer: dist_assets + "/css/style.css",
	},
	images: {
		input: src_assets + "/images/**/",
		output: dist_assets + "/images/",
		svg: src_assets + "/images/svg/*.png",
		exc: "!" + src_assets + "/images/sprite/*" + " !" + src + "/images/dataURI/*",
	},
	sprite: {
		input: src_assets + "/images/sprite/",
		output: dist_assets + "/images/sprite/",
		css: src_assets + "/sass/vendor/",
		html: src_assets + "/html/guide/sprite.html",
	},
	html: {
		input: src + "/**/*.html",
		output: dist + "/",
...
function htmlPUG() {
	gulp.src(path.pug.input).pipe(pug()).pipe(gulp.dest(path.pug.output)).on("finish", browserSync.reload);
}

//js minify
const minify = require("gulp-minify");
function min_js(done) {
	gulp.src([path.js.input])
		.pipe(
			minify({
				ext: {
					min: ".min.js",
...
function imgCopy(done) {
	gulp.src([path.images.input + "*.*", path.images.exc])
		.pipe(gulp.dest(path.images.output))
		.on("finish", browserSync.reload);
	done();
}
...
//clean-html
function htmlClean() {
	return del([path.html.output + "**/*.html"]);
}
...
function datauri(done) {
	gulp.src(path.images.input + "dataURI/*")
		.pipe(
			imageDataURI({
				template: {
